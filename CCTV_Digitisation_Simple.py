...
def main():
    ...
    try:
        ...
        for k in range(len(S_StartNode)):
            if mXS[k] == "0":
                lostMH.append(str(S_StartNode[k]))
            if mXE[k] == "0":
                lostMH.append(str(S_EndNode[k]))
        lostMH = list(set(lostMH))
        ...
        # Length of photo field
        lp = len(max(x, key=len))
        ...
        #Find the length of video names
        lv = len(max(Video, key=len))
        ...
        outputlostMH = '{0}.csv'.format(lostMHoutput)
        ...
        with open(outputlostMH,'wb') as output:
            writer = csv.writer(output)
            #loop through point list and write to CSV
            for item in lostMH:
                writer.writerow([item])
        ...
        out.field('Date','C')
        out.field('Video','C',lv)
        out.field('Infiltration','N',6)
        out.field('xcoord','C',11)
        out.field('ycoord','C',11)
